...
const validatorNameUser=(name)=>{
    if(/^([A-Za-z0-9\s]{8,})+$/g.test(name)){
        alert('el nombre de usuario tiene 8 a más dijitos y solo puede contener letras, números y espacios en blanco')
    return true
    }else{
        alert('el nombre de usuario debe tener 8 amás díjitos y solo puede contener letras, números y espacios en blanco');
        return false
    }
}
...
const validatorPassword = (password) => {
    ...
    if (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])([A-Za-z\d$@$!%*?&]|[^ ]){8,}$/.test(password)) {
       alert('la contraseña tiene más de 8 caracteres 1 min 1 may 1 simbolo [$@$!%*?&]');
        return true;
    } else {
...
const validatorEmailAndPassword =(email, password, name)=>{
    ...
    return true;
       ...
       alert('pasword OK ,name OK,correo es inválido');
        ...
        alert('correo  OK , contraseña ok,nombre invalid');
      return false
    }
